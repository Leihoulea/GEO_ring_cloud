...
| geo_ring_cloud_claas3_adapter.py | geo_ring_cloud |  | shared_library | common | CLAAS-3 recursive discovery, deterministic deduplication, decoding, unit conversion, QA, and per-variable masks |  |
| geo_ring_cloud_lineage.py | geo_ring_cloud |  | shared_library | common | Common run and artifact lineage manifest writer |  |
| geo_ring_cloud_run_discovery.py | geo_ring_cloud |  | shared_library | common | Matrix-manifest-first run discovery with legacy time-tag compatibility |  |
| geo_ring_cloud_source_registry.py | geo_ring_cloud |  | shared_library | common | Stable source IDs, processing streams, products, profiles, tolerances, and variable rules |  |
| path_config.py | geo_ring_cloud |  | shared_library | common | Central environment-overridable paths including GEO_RING_CLAAS3_ROOT |  |
...
| geo_ring_cloud_time_run_prune_failed.py | geo_ring_cloud |  | support |  | 当前文件系统扫描补充脚本 |  |
| geo_ring_cloud_time_run_prune_intermediates.py | geo_ring_cloud |  | support |  | 当前文件系统扫描补充脚本 |  |
| tests/_tmp/baseline_regression.py | geo_ring_cloud |  | support |  | 当前文件系统扫描补充脚本 |  |
