...
| --- | --- | --- | --- | --- | --- | --- | --- | --- | --- |
| D:\AAAresearch_paper\data | 强相关 | 原始卫星数据（FY4A/FY4B L1 + FY4B 云产品 CLM/CLP/CLT/CTH/CTP/CTT/GEO + H09_Data） | 10459 | 150.6 GB | 是 | 是 | 否 | keep_in_place | geo_ring_cloud.paths.HIMAWARI_R21_DIR；06f 扫描 data/ |
...
| D:\AAAresearch_paper\geo_ring_cloud_stage1_evidence_pack | 强相关 | Stage1 证据包（latest + 10 snapshots） | 308 | 1.2 MB | 是 | 是 | 否 | keep_in_place | geo_ring_cloud.evidence_pack:EVIDENCE_ROOT 产出 |
| D:\AAAresearch_paper\geo_ring_cloud_stage1_time_runs | 强相关 | 多时次运行、跨阶段实验与阶段诊断产物根目录 | 73379 | 453.7 GB | 是 | 是 | 否 | keep_in_place | 运行器与 stage_08 以后诊断脚本通过 RUNS_ROOT 引用；大产物保留原位 |
| D:\AAAresearch_paper\third_report\code\geo_ring_cloud_stage1 | 强相关 | GEO-ring Cloud 主代码、阶段脚本、共享组件与测试 | 179 | 2.8 MB | 是 | 是 | 否 | keep_in_place | 权威代码源；产物写入配置化的 stage/time-runs 根目录 |
...
| D:\AAAresearch_paper\third_report\code\geo_cloud_download | 上游相关 | GEO 云产品下载器（EUMETSAT API + S3） | 52 | 642.6 KB | 间接 | 是 | 否 | keep_in_place | 下载原始云产品到 data/ 与 E 盘 |
...
| D:\AAAresearch_paper\cloud | 弱相关 | 云产品参考文献 PDF（Zhao 等 2026 全天全球云物理属性） | 314 | 226.0 MB | 否 | 是 | 否 | document_only | 方法论参考，代码无路径引用 |
...
| D:\AAAresearch_paper\data\A202607010902106154 | 无关 | 空目录 |  | 0 | 否 | 否 | 否 | missing | 空 |
