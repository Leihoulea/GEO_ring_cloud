...
| D:\AAAresearch_paper\geo_ring_cloud_stage1_evidence_pack | 强相关 | Stage1 证据包（latest + 10 snapshots） | 280 | 887.0 KB | 是 | 是 | 否 | keep_in_place | geo_ring_cloud.evidence_pack:EVIDENCE_ROOT 产出 |
| D:\AAAresearch_paper\geo_ring_cloud_stage1_time_runs | 强相关 | 多时次运行、跨阶段实验与阶段诊断产物根目录 | 43737 | 229.0 GB | 是 | 是 | 否 | keep_in_place | 运行器与 stage_08 以后诊断脚本通过 RUNS_ROOT 引用；大产物保留原位 |
| D:\AAAresearch_paper\third_report\code\geo_ring_cloud_stage1 | 强相关 | GEO-ring Cloud 主代码、阶段脚本、共享组件与测试 | 173 | 2.7 MB | 是 | 是 | 否 | keep_in_place | 权威代码源；产物写入配置化的 stage/time-runs 根目录 |
...
| D:\AAAresearch_paper\cloud | 弱相关 | 云产品参考文献 PDF（Zhao 等 2026 全天全球云物理属性） | 90 | 40.6 MB | 否 | 是 | 否 | document_only | 方法论参考，代码无路径引用 |
