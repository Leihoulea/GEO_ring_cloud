...
| D:\AAAresearch_paper\data_check_report | 强相关 | 前序数据审计报告（REPORT_ROOT，00-00f 阶段证据源） | 250 | 887.2 MB | 是 | 是 | 否 | keep_in_place | stage1_common.py:25 REPORT_ROOT；PARSED_METADATA/MAPPING_YAML 等 |
...
| D:\AAAresearch_paper\geo_ring_cloud_stage1_time_runs | 强相关 | 多时次运行目录（48 批次 + 5 EPIC 专题） | 34816 | 156.6 GB | 是 | 是 | 否 | keep_in_place | 08e/08f/08g/08h/08i/08j/09 等脚本以 RUNS_ROOT 引用 |
| D:\AAAresearch_paper\third_report\code\geo_ring_cloud_stage1 | 强相关 | Stage1 主代码（41 个 .py，01-09 全流水线） | 88 | 1.7 MB | 是 | 是 | 否 | keep_in_place | ring cloud 真正的代码源；产物写到根 geo_ring_cloud_stage1/ |
...
| D:\AAAresearch_paper\cloud | 弱相关 | 云产品参考文献 PDF（Zhao 等 2026 全天全球云物理属性） | 2 | 1.2 MB | 否 | 是 | 否 | document_only | 方法论参考，代码无路径引用 |
