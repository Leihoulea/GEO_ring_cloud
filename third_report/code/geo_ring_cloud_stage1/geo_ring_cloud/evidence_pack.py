...
from geo_ring_cloud import PROJECT_ID
from geo_ring_cloud.lineage import code_commit
from geo_ring_cloud.paths import (
    CODE_ROOT,
    DATA_CHECK_ROOT,
    EVIDENCE_ROOT,
    GEOMETRY_ROOT,
...
def script_manifest_rows() -> list[list[str]]:
    script_status = {
        ...
        "stage_07p_overlap_validation/stage_07p_overlap_validator.py": ("07p", "executed canonical"),
        "stage_07p_overlap_validation/stage_07p_claas3_profile_pair_evaluation.py": ("07p", "present canonical gate"),
        "07p_overlap_validator_hotfix.py": ("07p", "compatibility entrypoint"),
...
def build_evidence_manifest(ts: str, snapshot_id: str) -> dict:
    ...
    return {
        ...
        "input_paths": [str(path) for path in source_roots],
        "output_paths": [str(LATEST_ROOT), str(SNAPSHOTS_ROOT / snapshot_id)],
        "parameter_summary": {"snapshot_id": snapshot_id, "evidence_pack_version": "3.0"},
        "timestamp_utc": ts,
