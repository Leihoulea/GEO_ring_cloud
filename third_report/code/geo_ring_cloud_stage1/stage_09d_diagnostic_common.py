# -*- coding: utf-8 -*-
"""Shared helpers for Stage 09d VIS/SEL diagnostic reruns.

These helpers deliberately reuse the existing Stage 09d full-pixel diagnostic
reader/mapping functions.  They do not modify Stage 05/06 production outputs.
"""
...
STAGE09D_HELPER_DIR = SCRIPT_DIR / "stage09d_full_pixel_diagnostics"
if str(STAGE09D_HELPER_DIR) not in sys.path:
    sys.path.insert(0, str(STAGE09D_HELPER_DIR))

import path_config  # noqa: E402
import run_stage09d_full_pixel_diagnostics as d09d  # noqa: E402
...
def classify(values: np.ndarray, bins: list[tuple[str, float, float]]) -> np.ndarray:
    return d09d.classify_array(values, bins)
...
def load_angle_on_epic(ctx: dict[str, Any], source: str, variable_keyword: str) -> tuple[np.ndarray, bool, str]:
    ...
    arrs = d09d.load_npz(hits[0])
    data = np.asarray(arrs["data"], dtype=np.float32)
    valid = np.asarray(arrs.get("valid_mask", np.isfinite(data))).astype(bool)
    sampled, sampled_valid = d09d.sample_grid(data, valid, ctx["epic"]["lat"], ctx["epic"]["lon"], ctx["grid"])
...
def scene_boundary(ctx: dict[str, Any], policy_name: str = "A_inclusive_binary") -> dict[str, np.ndarray]:
    frac, boundary = d09d.make_boundary(ctx["fused_on_epic"], ctx["fused_on_valid"], policy_name)
...
def policy_classes(ctx: dict[str, Any], policy_name: str) -> tuple[np.ndarray, np.ndarray, np.ndarray, np.ndarray]:
    ...
    geo_cls, geo_pv = d09d.apply_policy(ctx["fused_on_epic"], policy["geo"])
...
def metric_row(epic_cls: np.ndarray, geo_cls: np.ndarray, valid: np.ndarray, policy_name: str) -> dict[str, Any]:
    policy = d09d.POLICIES[policy_name]
    row = d09d.binary_metrics(epic_cls, geo_cls, valid, policy["positive"])
...
def source_samples(ctx: dict[str, Any]) -> tuple[dict[str, np.ndarray], dict[str, np.ndarray], list[str]]:
    ...
    warnings: list[str] = []
    for source in d09d.SOURCES:
        pref = d09d.find_prefusion(ctx["run_dir"], source, ctx["tag"])
        if pref is None:
            warnings.append(f"missing_prefusion_{source}")
            continue
        try:
            arrs = d09d.load_npz(pref)
            raw_valid = np.asarray(arrs.get("fusion_valid_mask", arrs.get("valid_mask", np.isfinite(arrs["data"])))).astype(bool)
            raw_on, raw_valid_on = d09d.sample_grid(arrs["data"], raw_valid, ctx["epic"]["lat"], ctx["epic"]["lon"], ctx["grid"])
            std = d09d.source_to_standard(source, raw_on)
            classes[source] = std
...
def apply_geo_policy(std: np.ndarray, valid: np.ndarray, policy_name: str) -> tuple[np.ndarray, np.ndarray]:
    policy = d09d.POLICIES[policy_name]
    cls, pv = d09d.apply_policy(std, policy["geo"])
    return cls, valid & pv
...
def write_run_manifest(path: Path, *, script_path: Path, input_paths: list[Path], output_paths: list[Path], filters: list[str], unit_conversions: list[dict[str, Any]], row_counts: dict[str, int], warnings: list[dict[str, Any]]) -> None:
    write_json(
        path,
        {
            "stage_id": STAGE_ID,
