...
from geo_ring_cloud.lineage import utc_now, write_manifest  # noqa: E402
from geo_ring_cloud.paths import BASE_STAGE_ROOT, CODE_ROOT, PROJECT_ROOT, RUNS_ROOT  # noqa: E402
...
def write_code_reuse_audit(out_root: Path) -> None:
    rows = [
        {"functionality": "EPIC L2 cloud read / variable discovery", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "read_epic_l2, find_var_name, read_nc_var", "can_reuse_directly": "yes", "needs_wrapper": "no", "risk": "low", "notes": "Used by existing 08c semantic comparison."},
        {"functionality": "EPIC time parsing", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "parse_time_from_name, target_delta_min", "can_reuse_directly": "yes", "needs_wrapper": "optional", "risk": "low", "notes": "09B mostly reuses existing Stage 08 inventory time fields."},
        {"functionality": "GEO-ring fused product read", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "load_npz_array", "can_reuse_directly": "yes", "needs_wrapper": "no", "risk": "low", "notes": "Reads fused npz without modifying fusion output."},
        {"functionality": "prefusion source cloud-mask read", "existing_script_path": str(CODE_ROOT / "08f_geometry_and_prefusion_epic_diagnostics.py"), "existing_function_or_class": "find_reprojected_cloud_mask, load_npz", "can_reuse_directly": "yes", "needs_wrapper": "yes", "risk": "low", "notes": "09B uses file presence and existing 08f outputs for source diagnostics."},
        {"functionality": "grid sampling to EPIC pixels", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "sample_grid_to_points", "can_reuse_directly": "yes", "needs_wrapper": "no", "risk": "medium", "notes": "Nearest-neighbor sampling is a diagnostic limitation, not fusion logic."},
        {"functionality": "cloud-mask semantic mapping", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "EPIC_POLICIES, apply_policy", "can_reuse_directly": "yes", "needs_wrapper": "no", "risk": "low", "notes": "Keeps Policy A/B/C identical to Stage 08."},
        {"functionality": "metric computation", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "binary_metrics, multiclass_agreement", "can_reuse_directly": "yes", "needs_wrapper": "no", "risk": "low", "notes": "Agreement/F1/IoU remain diagnostic, not truth validation."},
        {"functionality": "quicklook plotting", "existing_script_path": str(CODE_ROOT / "08c_epic_cloud_mask_semantic_sensitivity.py"), "existing_function_or_class": "quicklook functions in 08c", "can_reuse_directly": "partly", "needs_wrapper": "yes", "risk": "medium", "notes": "09B only adds summary figures backed by CSV."},
        {"functionality": "source-by-source diagnostics", "existing_script_path": str(CODE_ROOT / "08f_geometry_and_prefusion_epic_diagnostics.py"), "existing_function_or_class": "run", "can_reuse_directly": "yes", "needs_wrapper": "yes", "risk": "medium", "notes": "Existing 08f coverage may lag newly generated samples; warnings are recorded."},
        {"functionality": "source-pair overlap diagnostics", "existing_script_path": str(CODE_ROOT / "08j_prefusion_source_pair_overlap_diagnostics.py"), "existing_function_or_class": "analyze_sample, build_report", "can_reuse_directly": "yes", "needs_wrapper": "yes", "risk": "medium", "notes": "Used only for diagnosis; never generates fused products."},
        {"functionality": "single-sample pipeline runner", "existing_script_path": str(CODE_ROOT / "run_epic_georing_single_sample.py"), "existing_function_or_class": "run_pipeline", "can_reuse_directly": "yes", "needs_wrapper": "yes", "risk": "medium", "notes": "09B calls it through conda run -n pytorch python -B."},
        {"functionality": "batch runner", "existing_script_path": str(CODE_ROOT / "run_epic_georing_sample_batch.py"), "existing_function_or_class": "run_one, build_report", "can_reuse_directly": "partly", "needs_wrapper": "yes", "risk": "medium", "notes": "09B implements stricter March-only target selection and richer logging."},
        {"functionality": "Stage 09 summary", "existing_script_path": str(CODE_ROOT / "stage_09" / "stage_09_run_epic_geo_ring_cloud_mask_diagnostics.py"), "existing_function_or_class": "main", "can_reuse_directly": "yes", "needs_wrapper": "yes", "risk": "low", "notes": "Can be rerun after new samples; 09B keeps separate outputs."},
    ]
...
def run_attempts(out_root: Path, targets: list[dict[str, Any]], args: argparse.Namespace) -> list[dict[str, Any]]:
    ...
    for row in targets[: max(args.max_run_attempts, 0)]:
        ...
        with log_path.open("w", encoding="utf-8", errors="replace") as log:
            ...
            proc = subprocess.run(cmd, cwd=str(CODE_ROOT), stdout=log, stderr=subprocess.STDOUT, text=True)
...
def write_stage_manifest(out_root: Path, args: argparse.Namespace, report: Path) -> Path:
    outputs = [
        out_root / "00_code_reuse_audit" / "stage09b_existing_code_reuse_audit.csv",
        out_root / "01_inventory_expansion" / "stage09b_full_candidate_inventory_202403.csv",
        out_root / "02_target_selection" / "stage09b_overnight_target_list.csv",
        out_root / "03_stage_run_attempts" / "stage09b_stage_run_attempt_status.csv",
        out_root / "03_stage_run_attempts" / "stage09b_stage_run_attempt_history.csv",
        out_root / "04_expanded_core_metrics" / "stage09b_sample_level_semantic_summary.csv",
        out_root / "13_summary_figures" / "stage09b_plot_index.csv",
        report,
    ]
    return write_manifest(
        out_root / "reports" / "stage_09b_full_overnight_manifest.json",
        canonical_stage_id=STAGE_ID,
        generating_script=Path(__file__),
        input_paths=[STAGE08_INVENTORY, STAGE09_INVENTORY, args.base_stage_root],
        ...
        source_profile="march_2024_full_overnight",
        extra={
            "artifact_naming_schema": "legacy_stage09b_v1",
            "artifact_naming_note": "Historical artifact names are retained for resume and Stage 09c compatibility.",
        },
    )
...
def main() -> int:
    ...
    write_stage_manifest(out_root, args, report)
