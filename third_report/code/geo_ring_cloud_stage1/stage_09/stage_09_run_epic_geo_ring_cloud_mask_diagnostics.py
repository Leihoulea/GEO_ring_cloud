...
def write_stage_manifest(out_dir: Path, args: argparse.Namespace, report: Path) -> Path:
    outputs = [
        out_dir / "stage09_epic_product_inventory.csv",
        out_dir / "stage09_fused_vs_epic_semantic_metrics_long.csv",
        out_dir / "stage09_sample_level_semantic_summary.csv",
        out_dir / "stage09_group_semantic_summary.csv",
        out_dir / "stage09_strata_summary.csv",
        out_dir / "stage09_prefusion_source_summary.csv",
        out_dir / "stage09_source_pair_summary.csv",
        out_dir / "stage09_diagnostic_factor_table.csv",
        out_dir / "stage09_plot_index.csv",
        out_dir / "stage09_warnings.csv",
        report,
    ]
    return write_manifest(
        out_dir / "stage_09_epic_geo_ring_cloud_mask_diagnostics_manifest.json",
        canonical_stage_id=STAGE_ID,
        generating_script=Path(__file__),
        input_paths=[SELECTION_INVENTORY, MULTISAMPLE_DIR, GEOM_PREFUSION_DIR, TIME_OFFSET_PAIR_DIR, args.runs_root],
        output_paths=outputs,
        parameters={"runs_root": args.runs_root, "out_dir": args.out_dir},
        project_root=PROJECT_ROOT,
        run_id=out_dir.name,
        source_profile="epic_geo_ring_cloud_mask_diagnostics",
        extra={
            "artifact_naming_schema": "legacy_stage09_v1",
            "artifact_naming_note": "Historical artifact names are retained for Stage 09b/09c compatibility.",
        },
    )
...
def main() -> int:
    ...
    write_stage_manifest(out_dir, args, report)
