...
def load_module(path: Path, name: str) -> Any:
    ...
    if spec is None or spec.loader is None:
        raise RuntimeError(f"cannot import diagnostic support: {path}")
    module = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(module)
    return module


def stage10_support() -> Any:
    return load_module(SCRIPT_DIR / "stage_10_cth_validation" / "stage_10_run_cth_validation.py", "stage10_aligned_support")
...
def read_epic(path: Path) -> dict[str, Any]:
    support = stage10_support()
    a = support.read_epic(path, "geophysical_data/A-band_Effective_Cloud_Height")
    b = support.read_epic(path, "geophysical_data/B-band_Effective_Cloud_Height")
    return {
        "lat": a["lat"],
        "lon": a["lon"],
