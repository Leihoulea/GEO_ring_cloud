    ...
    with master.open("a", encoding="utf-8", errors="replace") as mlog:
        ...
        for target in targets:
            ...
            with log_file.open("w", encoding="utf-8", errors="replace") as log:
                ...
                try:
                    proc = subprocess.run(cmd, cwd=str(CODE_ROOT), env=env, stdout=log, stderr=subprocess.STDOUT, text=True, timeout=args.timeout_seconds)
...
def write_stage_manifest(out_root: Path, args: argparse.Namespace, report: Path) -> Path:
    ...
    return write_manifest(
        out_root / "reports" / "stage_09c_scaled_batch_manifest.json",
        canonical_stage_id=STAGE_ID,
        generating_script=Path(__file__),
        input_paths=[INV_09B, TARGET_09B, STAGE09_ROOT, args.base_stage_root],
        ...
        source_profile="march_2024_scaled_batch",
        extra={
            "artifact_naming_schema": "legacy_stage09c_v1",
            "artifact_naming_note": "Historical artifact names are retained for resume and downstream compatibility.",
        },
    )
