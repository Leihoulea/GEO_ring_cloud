...
Write-GeoRingPresentationManifest `
  -Path (Join-Path $OutDir "geo_ring_cloud_epic_group_meeting_manifest.json") `
  -GeneratingScript $PSCommandPath `
  -InputPaths @($SummaryRoot) `
  -OutputPaths @($pptxPath, $pdfPath, $notesPath) `
  -Parameters @{ language = "en"; summary_root = $SummaryRoot; output_dir = $OutDir } `
