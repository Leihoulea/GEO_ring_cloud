...
. (Join-Path $PSScriptRoot "geo_ring_cloud_presentation_manifest.ps1")
...
if (-not $PSBoundParameters.ContainsKey("SlideJson")) {
  $SlideJson = Join-Path $PSScriptRoot "geo_ring_cloud_epic_group_meeting_slides_cn.json"
}
...
Write-GeoRingPresentationManifest `
  -Path (Join-Path $OutDir "geo_ring_cloud_epic_group_meeting_cn_manifest.json") `
  -GeneratingScript $PSCommandPath `
  -InputPaths @($SummaryRoot, $SlideJson) `
  -OutputPaths @($pptxPath, $pdfPath, $notesPath) `
  -Parameters @{ language = "zh-CN"; summary_root = $SummaryRoot; slide_json = $SlideJson; output_dir = $OutDir } `
  -ProjectRoot $GeoRingProjectRoot
