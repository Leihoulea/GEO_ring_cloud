...
from geo_ring_cloud.paths import EXTERNAL_EPIC_L2_ROOT, PROJECT_ROOT, RUNS_ROOT
...
def write_stage_manifest(report: Path) -> Path:
    inputs = [
        SUMMARY_ROOT / "epic_georing_multisample_summary.csv",
        SUMMARY_ROOT / "epic_georing_multisample_source_metrics.csv",
        SUMMARY_ROOT / "epic_georing_multisample_quicklook_index.csv",
        TARGET_SELECTION_ROOT / "epic_202403_geo_source_candidate_inventory.csv",
        TARGET_SELECTION_ROOT / "recommended_epic_georing_validation_targets.csv",
        SUMMARY_ROOT / "geometry_prefusion_diagnostics",
        TIME_CONTROL_ROOT,
        EXTERNAL_EPIC_L2_ROOT,
    ]
    return write_manifest(
        report.parent / "stage_08k_consolidated_report_manifest.json",
        canonical_stage_id=STAGE_ID,
        generating_script=Path(__file__),
        input_paths=inputs,
        output_paths=[report],
        parameters={"report_language": "zh-CN", "report_scope": "stage_08_integrated"},
        project_root=PROJECT_ROOT,
        run_id=report.parent.name,
        source_profile="stage_08_integrated_reporting",
        extra={
            "artifact_naming_schema": "legacy_stage08_v1",
            "artifact_naming_note": "The historical report filename is retained for evidence continuity.",
        },
    )
...
def run() -> Path:
    ...
    write_stage_manifest(OUT_REPORT)
    return OUT_REPORT


def build_parser() -> argparse.ArgumentParser:
    return argparse.ArgumentParser(description="Stage 08k integrated EPIC/GEO-ring report builder.")
