...
from geo_ring_cloud.diagnostics.full_pixel_workflow import bool_series, read_csv  # noqa: E402
...
def build_valid_count_pixel_diagnostics() -> tuple[pd.DataFrame, pd.DataFrame]:
    manifest = read_csv(ROOT / "00_sample_manifest" / "stage09d_53_sample_manifest.csv")
    bins = [("1", 1, 2), ("2", 2, 3), ("3", 3, 4), (">=4", 4, 100)]
    rows = []
    selected_rows = []
    for row in manifest:
        if not bool_series(row.get("can_run_source_pair")):
            continue
        ctx = full_pixel.sample_context(row)
        epic = ctx["epic"]
        epic_cls, epic_pv = full_pixel.apply_policy(epic["cloud_mask"], full_pixel.POLICIES["A_inclusive_binary"]["epic"])
        valid_earth = np.isin(epic["cloud_mask"], [1, 2, 3, 4])
        labels = full_pixel.classify_array(ctx["valid_count"], bins)
        fused_cls, fused_pv = full_pixel.apply_policy(ctx["fused_on_epic"], full_pixel.POLICIES["A_inclusive_binary"]["geo"])
        fused_base = valid_earth & epic_pv & ctx["fused_on_valid"] & fused_pv
        source_id_to_name = {0: "none", **full_pixel.SOURCE_ID}
        ...
        for source in full_pixel.SOURCES:
            pref = full_pixel.find_prefusion(ctx["run_dir"], source, row["sample_id"])
            if pref is None:
                continue
            ...
            raw_on, raw_valid_on = full_pixel.sample_grid(arrs["data"], raw_valid, epic["lat"], epic["lon"], ctx["grid"])
            std = full_pixel.source_to_standard(source, raw_on)
            geo_cls, geo_pv = full_pixel.apply_policy(std, full_pixel.POLICIES["A_inclusive_binary"]["geo"])
            base_valid = valid_earth & epic_pv & raw_valid_on & (std >= 0) & geo_pv
            for label, _, _ in bins:
                valid = base_valid & (labels == label)
                if int(np.count_nonzero(valid)) == 0:
                    continue
                m = full_pixel.binary_metrics(epic_cls, geo_cls, valid, full_pixel.POLICIES["A_inclusive_binary"]["positive"])
                rows.append(
                    {
                        "sample_id": row["sample_id"],
                        "source_name": source,
