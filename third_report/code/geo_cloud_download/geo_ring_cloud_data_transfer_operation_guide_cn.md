...
自动上传模式默认用 2 路受限并行在服务器端读取并计算 SHA-256（可在命令行用
`--server-verify-workers 1` 至 `4` 调整）。这是针对 `/data04` 网络存储的保守起点；
仪表板会显示校验文件数、字节数、百分比和当前文件。校验与下一批的本地下载可并行，
但不会修改或删除任何本地原始文件。
...
3. 中断后按服务器 `.part` 大小续传；
4. 完成后原子改名为正式文件；
5. 在服务器逐文件计算 SHA-256；复核期间上传器每 5 秒取回一次轻量进度记录，仪表盘显示已复核文件数、按字节计算的进度和当前文件；
6. 队列调度只等待前一批的本地下载完成，不等待服务器复核，因此空间门禁通过时下一批下载可以与上一批的只读服务器复核并行；
7. 把 `server_verification.json` 自动取回本地；只有复核全部通过时才显示 `PASS`。
