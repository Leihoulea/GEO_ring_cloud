...
MUST use `geo_ring_cloud.lineage.write_manifest`. Treat `code_commit` only as
the repository HEAD at manifest-write time, not as proof that HEAD contains the
executed script. The manifest MUST retain the generating-script SHA-256, Git
state, worktree/commit blobs, and `commit_represents_script`. A false value is a
lineage warning that must not be hidden.
...
Geo Ring Cloud Python code MUST use `geo_ring_cloud.paths`; `path_config.py`
is legacy-import compatibility only. PowerShell orchestration MUST dot-source
`geo_ring_cloud_path_configuration.ps1` or use the same `GEO_RING_*`
environment-variable contract.

MUST NOT add machine-local drive paths outside the two canonical Python and
PowerShell path-configuration files. This includes `D:`, `E:`, `F:`, Conda
installation paths, and report prose generated by code.
...
New stage code MUST live below
`third_report/code/geo_ring_cloud_stage1/<canonical-stage-directory>`. New stage
outputs MUST live below
`geo_ring_cloud_stage1_time_runs/<canonical-stage-run-directory>`. MUST NOT put
stage-owned directories at repository root.

`geo_ring_cloud_stage1/reports` is a frozen legacy shared report pool. New stage
scripts MUST NOT import or write through `REPORT_DIR`. Historical source
snapshots belong under
`geo_ring_cloud_stage1_evidence_pack/source_snapshots`; new stage scripts MUST
NOT import or write through the legacy `SCRIPT_DIR` alias.
